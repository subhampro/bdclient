...
local blips = {
	-- 60KM/H ZONES
	{title="Speedcamera (60KM/H)", colour=1, id=1, x = 195.15, y = -811.33, z = 31.08},
	{title="Speedcamera (60KM/H)", colour=1, id=2, x = 263.49, y = -868.4, z = 29.24}, -- 60KM/H ZONE
	
	-- 80KM/H ZONES
	--{title="Speedcamera (80KM/H)", colour=1, id=1, x = 9506.0671, y = 4145.2431, z = 388.1054}, -- 80KM/H ZONE
	
	-- 120KM/H ZONES
	--{title="Speedcamera (120KM/H)", colour=1, id=1, x = 190.4336, y = -840.7534, z = 30.9597}, -- 120KM/H ZONE
	--{title="Speedcamera (120KM/H)", colour=1, id=1, x = 273.5002, y = -838.6501, z = 29.2659}, -- 120KM/H ZONE
...
local Speedcamera60Zone = {
    {x = 195.15, y = -811.33, z = 31.08},
    {x = 263.49, y = -868.4, z = 29.24}
}

local Speedcamera80Zone = {
    -- {x = 2506.0671,y = 4145.2431,z = 38.1054},
    -- {x = 679.49,y = -412.89,z = 41.32},
    -- {x = 443.4,y = -2314.29,z = 63.36},
    -- {x = 401.27,y = -954.07,z = 29.43},
    -- {x = 398.45,y = -1050.63,z = 29.4},
    -- {x = 101.57,y = -1003.36,z = 29.41},
    -- {x = 103.4,y = -572.75,z = 31.51},
    -- {x = -180.98,y = -887.76,z = 29.35}
...
local Speedcamera120Zone = {
    ...
    -- {x = -2731.76,y = 8.07,z = 15.24 },
    -- {x = -2433.74,y = 3816.88,z = 22.04 }
					...
					if IsPedInAnyVehicle(playerPed, false) then
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then
							if hasBeenCaught == false then
								...
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "50THPOLSTANG" then -- BLACKLISTED VEHICLE
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "2015WBPOLSTANG" then -- BLACKLISTED VEHICLE
								...
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "1200RT" then -- BLACKLISTED VEHICLE
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "dodgeEMS" then -- BLACKLISTED VEHICLE
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "fd1" then -- BLACKLISTED VEHICLE
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "fd2" then -- BLACKLISTED VEHICLE
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "fd4" then -- BLACKLISTED VEHICLE
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (60KM/H)', 50) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then					
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (80KM/H)', 100) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then 
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (120KM/H)', 500) -- Sends a bill from the police
									else
